...
func newTestServer(t *testing.T) (*iris.Application, *fakeCartService, string) {
	t.Helper()
	app := iris.New()
	cartService := &fakeCartService{}
	routes.Setup(app, cartService)
...
func TestGetCurrentCart(t *testing.T) {
	app, service, token := newTestServer(t)
