...
- Added shared frontend auth features under `shared-components/frontend/package/auth-features` for auth services, guards, runtime config, interceptors, layouts, login, and forgot password pages.
- Refactored user, product, and order admin web apps to consume the shared auth feature package while keeping app-specific mock API handling.
- Updated frontend artifact hashing to include shared admin and auth feature packages so cached web app artifacts rebuild when shared frontend sources change.
- Ignored `.frontend-artifacts/` output generated by frontend artifact restore and publish workflows.
